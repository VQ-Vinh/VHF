...
from prana_elex.core.config.schema import TranslationConfig
...
## TRANSLATION
- The 'translation' field MUST be in the target language below.
- Translate ALL detected speech into the target language.
...
class PromptBuilder:
    ...
    def build_system_prompt(self) -> str:
        ...
        if src == "auto" or src == tgt:
            translation_instructions = f"Target language: {LANGUAGE_NAMES.get(tgt, tgt)}. The output in 'translation' field MUST be in {LANGUAGE_NAMES.get(tgt, tgt)}."
        else:
            translation_instructions = f"Translate from {LANGUAGE_NAMES.get(src, src)} to {LANGUAGE_NAMES.get(tgt, tgt)}. The output in 'translation' field MUST be in {LANGUAGE_NAMES.get(tgt, tgt)}."

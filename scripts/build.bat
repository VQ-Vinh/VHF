@echo off
chcp 65001 >nul
title PRANA ELEX — Build

set ROOT=%~dp0..
set SPEC=%~dp0PRANA_ELEX.spec

if not exist "%ROOT%\venv\Scripts\python.exe" (
    echo [ERROR] venv not found. Run setup.bat first.
    pause
    exit /b 1
)

echo [1/3] Installing PyInstaller...
"%ROOT%\venv\Scripts\pip.exe" install pyinstaller >nul 2>&1

echo [2/3] Cleaning old dist...
if exist "%ROOT%\dist\PRANA_ELEX" rmdir /s /q "%ROOT%\dist\PRANA_ELEX"
if exist "%ROOT%\dist\PRANA_ELEX.exe" del "%ROOT%\dist\PRANA_ELEX.exe"

echo [3/3] Building PRANA_ELEX.exe (cached build/)...
"%ROOT%\venv\Scripts\python.exe" -m PyInstaller "%SPEC%"

echo.
echo Done! Output: %ROOT%\dist\PRANA_ELEX\PRANA_ELEX.exe
pause

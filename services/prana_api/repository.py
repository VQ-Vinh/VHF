...
class FirestoreRepository:
    ...
    def _station_from_projection(station_id: str, data: dict) -> Station:
        ...
        return Station(
            ...
            command_failed_generation=int(data.get("command_failed_generation", 0)),
            command_error=data.get("command_error"),
            session_id=data.get("session_id", ""),
            sequence=int(data.get("sequence", 0)),
            last_seen_at=data.get("last_seen_at"),
            capabilities=data.get("capabilities"),
            active_capture_mode=data.get("active_capture_mode", "device"),
            active_audio_device_id=data.get("active_audio_device_id", ""),
            last_error=data.get("last_error"),
            retrying=bool(data.get("retrying", False)),
            retry_code=data.get("retry_code"),
            retry_attempt=int(data.get("retry_attempt", 0)),
            tx_state=data.get("tx_state", "idle"),
            tx_job_id=data.get("tx_job_id", ""),
            active_tx_audio_device_id=data.get("active_tx_audio_device_id", ""),

...
def _backend(config) -> BackendClient:
...
async def run_realtime(config_path: Path, target: str | None = None, capture_mode: str = "device", device_index: int = -1, data_dir: Path | None = None) -> None:
    ...
    orchestrator = PipelineOrchestrator(config, _backend(config), WASAPIBackend)
...
def main() -> None:
    ...
    if sys.stdin is None or args.gui:
        from prana_windows.desktop import main as desktop_main
        desktop_main()
        return
    ...
    if args.batch and args.batch[0] == "batch":
        ...
        run_batch(DEFAULT_CONFIG, files, target, data_dir=data_dir)
        return
    ...
    try:
        if args.batch:
            ...
            asyncio.run(run_realtime(DEFAULT_CONFIG, target, mode, dev_idx, data_dir=data_dir))
    except KeyboardInterrupt:
        pass

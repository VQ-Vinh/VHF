...
set "INSTALLER_DIR=%ROOT%\installers\windows"
cd /d "%ROOT%"
...
"%VENV%\Scripts\python.exe" "%ROOT%\tools\packaging\validate_client_config.py" "%ROOT%\apps\windows\config\default.toml"
if errorlevel 1 goto :error
...
if defined ISCC (
    "%ISCC%" /DMyAppVersion=%APP_VERSION% "%ROOT%\apps\windows\packaging\installer\PRANA_ELEX.iss"
    if errorlevel 1 goto :error
    echo [OK] Installer: %INSTALLER_DIR%\PRANA_ELEX_Setup_%APP_VERSION%_x64.exe
) else (

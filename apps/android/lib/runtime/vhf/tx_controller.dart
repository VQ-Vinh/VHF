...
import 'package:prana_mobile/runtime/vhf/tx_state.dart';
...
class TxController extends ChangeNotifier {
  ...
  final _monitorDelay = CancellableDelay();
  ...
  Future<void> stopRecording() async {
    ...
    _stopRecordingTimer();
    final operation = _operation;
    state = state.copyWith(phase: TxPhase.processing);
    notifyListeners();
    final duration =
  ...
  Future<void> cancelRecordingForBackground() async {
    if (_disposed || state.phase != TxPhase.recording) return;
    _stopRecordingTimer();
    final operation = ++_operation;
    _cancelling = true;
    state = TxState(targetLanguage: state.targetLanguage);
  ...
  Future<void> _monitorTransmission(TxDraft draft) async {
    ...
    while (!_disposed && _monitoredDraftId == draft.id) {
      await _monitorDelay.wait(retryDelay);
      if (_disposed || _monitoredDraftId != draft.id) return;
      TxDraft current;
      try {
  ...
  void dispose() {
    ...
    if (repository is TxRepositoryLifecycle) {
      (repository as TxRepositoryLifecycle).close();
    }
    _completionTimer?.cancel();
    _stopRecordingTimer();
